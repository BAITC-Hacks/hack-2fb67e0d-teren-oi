...
Приложение работает без ключа: точное сравнение пунктов и экспорт остаются доступны. AI-модель по умолчанию — `gpt-5.6-terra`. Чтобы включить AI-анализ, создайте **локальный** `.env` по образцу `.env.example` и задайте `OPENAI_API_KEY`; при необходимости поменяйте `OPENAI_MODEL`. После изменения `.env` перезапустите Python API. В интерфейсе AI включается отдельно для каждого анализа.
